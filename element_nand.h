...
private:
	...
	ELEMENT_NAND(ID2D1HwndRenderTarget*, BRUSH_SET*, IDWriteTextFormat*, float pos_x, float pos_y, float width, float height, unsigned id, unsigned input_amount);
	~ELEMENT_NAND() {}
...
public:
	static ELEMENT_NAND* Create(ID2D1HwndRenderTarget*, BRUSH_SET*, IDWriteTextFormat*, float pos_x, float pos_y, float width, float height, unsigned id, unsigned input_amount);
